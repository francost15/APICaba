...
# 6.Modificar el servicio de Usuarios para que devuelva los que tiene rol de capturista y con estado Activo
@router.get("/usuarios/roles_capturista/", response_model=list[UsuarioInDB])
async def leer_usuarios_roles_capturista():
    query = Usuarios.select().where(Usuarios.c.id_rol == 657900).where(Usuarios.c.status == 'activo')
